...
def ensemble_prediction(test_example, 
    ...
    if aggregation_method == "confidence":
        ...
        # then select the prediction from the model with the highest confidence score as the final prediction
        confidence_score = []
        i = 0
        task_list = list(task_models.keys()) # ['1', '2', '3', '4', '5']
        for task in task_list:
            outputs = task_models[task](**test_example)
            #pred_dist = model.predict_proba(test_example.reshape(1, -1))[0]
            prediction = outputs.logits.argmax(dim=-1)
            probabilities = torch.softmax(outputs.logits, dim=-1)
            pred_probs, pred_classes = probabilities.topk(k=1, dim=1)
            conf_scores = pred_probs.squeeze().tolist()  # convert the tensor to a list of confidence scores
        ...
        #prediction = task_models[best_model_index].predict(test_example.reshape(1, -1))
        for index, task in enumerate(task_list):
    ...
    elif aggregation_method == "neighbor_majority": #extra credit
...
def main():
    ...
    for aggregation_method in ["confidence", "entropy"]: #, "entropy", "neighbor_majority"
