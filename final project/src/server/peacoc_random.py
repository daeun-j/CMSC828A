...
from src.client.peacoc import PeacocClient, receive_data
import torch
...
server_sigma = random.uniform(0.1, 3)
...
class PeacocServer(FedAvgServer):
    ...
    def aggregate(self, gFval_params_cache, gFtrg_params_cache, beta_weight_cache, client_ids):                            
        ...
        if self.args.ab == 'a' or self.args.ab == 'B': 
            ...
            global server_sigma
            global server_mu
            server_sigma =  random.uniform(0.1, 3)  # Standard deviation of the Gaussian function)
            server_mu = random.uniform(1e-8,1e-10)  # Mean of the Gaussian function
            # Pass the updated value to the client
            receive_data(server_sigma, server_mu)
            # mu = 1e-9 # Mean of the Gaussian function
            # sigma = 0.1 # Standard deviation of the Gaussian function
            self.prealpha = self.alpha - (1/self.T) * self.ga * \
                self.softmaxwT(self.alpha, self.T) * (1-self.softmaxwT(self.alpha,  self.T)) * \
                    random.gauss(server_mu, server_sigma)
            self.alpha = self.softmaxwT(self.prealpha, self.T)
            print("server_sigma: ", server_sigma, " server_mu: ", server_mu," alpha: ", self.alpha)
