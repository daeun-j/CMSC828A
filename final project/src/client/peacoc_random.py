...
client_sigma = random.uniform(0.1, 3)
client_mu = random.uniform(1e-4,1e-6)

def receive_data(sig, mu):
...
class PeacocClient(FedAvgClient):
            ...
            for x, y in dataloader:
                ...
                if training_model == self.pers_model:
                    for pers_param, global_param in zip(
                        ...
                        if(self.beta == 0.0):
                            self.beta = torch.tensor([0.0], dtype=torch.float32)
                        beta = self.beta.item()
    ...
    def fit(self):
        ...
        if self.args.ab == 'b' or self.args.ab == 'B': 
            ...
            client_sigma =  random.uniform(0.1, 3) # Standard deviation of the Gaussian function)
            client_mu = random.uniform(1e-2,1e-4)  # Mean of the Gaussian function
            self.rd = random.gauss(client_mu, client_sigma)
            # while self.rd < 0:
            #     self.rd = random.gauss(client_mu, client_sigma)  # Keep generating until a positive number is obtained
            #new_beta = self.beta - self.gb 
            new_beta = self.rd 
            # if new_beta >= 0 :
            #     new_beta = abs(self.rd * cos)
            gb = deepcopy(self.gb)
            self.gb = gb
            
            if new_beta >= 0:
                #self.beta = new_beta 
                uddated_beta = self.rd * cos
                if uddated_beta >= 0 :
                    self.beta = uddated_beta
              
     
            print("self.beta: ", self.beta, " self.rd: ", self.rd, " cos: ", cos )

        pers_loss = self._fit( self.pers_model, self.trainloader, self.local_epoch)
        val_loss = self._fit(self.pers_model, self.valloader, self.local_epoch)
