...
max_iters = 14000*10+1
batch_size = 32
PATH = 'models/model_mnli_long_2'
# Load the tokenizer
...
task_sampler = TaskSampler(dataloader_dict=dataloader_dict, max_iters=max_iters, task_weights=[1, 0])
# task_sampler = TaskSampler(dataloader_dict=dataloader_dict, task_weights=[1, 0])# {mnli,ner}


optimizer = torch.optim.AdamW(model.parameters(),lr=2e-5,eps=1e-6)
criterion = nn.CrossEntropyLoss()
epoch_loss = []
task_sampler.set_sample_task([1, 0])
for idx, (task, batch)in enumerate(task_sampler):
    ...
    print(idx, task, loss, task_sampler.get_sample_task)
    loss.backward()
    #function check
